...
if (show_tabs) {
    ...
    for (var i = 0; i < array_length(tabs); i++) {
        ...
        draw_rectangle(tab_x, tab_y, tab_x + tab_width - 1, tab_y + tab_height, false); // 5 ÷ 4 ≈ 1

        // Tab border
        draw_set_alpha(1);
        draw_set_color(is_active ? color_border : c_gray);
        draw_rectangle(tab_x, tab_y, tab_x + tab_width - 1, tab_y + tab_height, true);
        ...
        if (has_item) {
            ...
            if (item_data != undefined) {
                ...
                if (show_quantity && item.quantity > 1) {
                    ...
                    draw_set_color(c_black);
                    draw_circle(
                        slot_x + scale_offset + scaled_size - 4, // 15 ÷ 4 ≈ 4
                        slot_y + scale_offset + scaled_size - 4, // 15 ÷ 4 ≈ 4
                        3, // 12 ÷ 4 = 3
                        false
                    );
                    draw_set_alpha(1);
                ...
                if (item_data.category == "toy" && item.durability != undefined && item.durability >= 0) {
                    ...
                    var bar_height = 1; // 4 ÷ 4 = 1
                    var bar_x = slot_x + scale_offset + (scaled_size - bar_width) / 2;
                    var bar_y = slot_y + scale_offset + scaled_size - 2; // 8 ÷ 4 = 2
...
if (show_tooltips && hover_slot != -1 && hover_slot < array_length(items)) {
    ...
    if (item_data != undefined) {
        ...
        // Tooltip dimensions
        var tooltip_width = 62;  // 250 ÷ 4 ≈ 62
        var tooltip_height = 37; // 150 ÷ 4 ≈ 37
        var tooltip_x = clamp(mx + 5, 0, display_get_gui_width() - tooltip_width);   // 20 ÷ 4 = 5
        var tooltip_y = clamp(my + 5, 0, display_get_gui_height() - tooltip_height);  // 20 ÷ 4 = 5
        ...
        draw_set_valign(fa_top);
        draw_set_color(c_white);
        var text_x = tooltip_x + 2;  // 10 ÷ 4 ≈ 2
        var text_y = tooltip_y + 2;  // 10 ÷ 4 ≈ 2
        var line_height = 5;         // 18 ÷ 4 ≈ 5

        // Name
        draw_text(text_x, text_y, item_data.name);
        text_y += line_height + 1; // 5 ÷ 4 ≈ 1
        ...
        // Description
        draw_text_ext(text_x, text_y, item_data.description, line_height, tooltip_width - 5); // 20 ÷ 4 = 5
...
draw_text(ui_x + ui_width - 5, ui_y + ui_height - 6, storage_text); // 20 ÷ 4 = 5, 25 ÷ 4 ≈ 6

// Draw scroll indicator
if (max_scroll > 0) {
    draw_set_halign(fa_center);
    var scroll_text = "Scroll: " + string(scroll_offset) + "/" + string(max_scroll);
    draw_text(ui_x + ui_width / 2, ui_y + ui_height - 6, scroll_text); // 25 ÷ 4 ≈ 6
}
