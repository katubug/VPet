...
draw_text(ui_x + 5, ui_y + 2, ui_title); // 20→5, 10→2

// Draw corn count
draw_set_halign(fa_right);
draw_set_color(c_yellow);
draw_text(ui_x + ui_width - 12, ui_y + 2, "Corns: " + string(global.game.corns)); // 50→12, 10→2
...
var grid_start_y = ui_y + 20 + slot_padding; // 80→20
...
for (var row = 0; row < grid_rows; row++) {
    for (var col = 0; col < grid_columns; col++) {
        ...
        draw_sprite_ext(
            ...
            slot_y + scale_offset + scaled_size / 2 - 2, // 10→2
            sprite_scale,
            sprite_scale,
            0,
            c_white,
        ...
        draw_rectangle(
            slot_x + scale_offset,
            slot_y + scale_offset + scaled_size - 5, // 20→5
            slot_x + scale_offset + scaled_size,
            slot_y + scale_offset + scaled_size,
            false
        );
        ...
        draw_text(
            slot_x + scale_offset + scaled_size / 2,
            slot_y + scale_offset + scaled_size - 1, // 2→1
        ...
        if (shop_item.stock > 0) {
            ...
            draw_set_alpha(0.8);
            draw_text(
                slot_x + scale_offset + scaled_size - 1, // 5→1
                slot_y + scale_offset + 1,               // 5→1
...
if (!global.dialog_open && hover_slot != -1 && hover_slot < array_length(shop_items)) {
    ...
    if (item_data != undefined) {
        ...
        var tooltip_height = 30; // 120 ÷ 4 = 30
        var tooltip_x = clamp(mx + 5, 0, display_get_gui_width() - tooltip_width);   // 20→5
        var tooltip_y = clamp(my + 5, 0, display_get_gui_height() - tooltip_height);  // 20→5
        ...
        draw_set_valign(fa_top);
        draw_set_color(c_white);
        var text_x = tooltip_x + 2;  // 10→2
        var text_y = tooltip_y + 2;  // 10→2
        var line_height = 5;         // 18→5

        // Name
        draw_text(text_x, text_y, item_data.name);
        text_y += line_height + 1; // 5→1
        ...
        // Description
        draw_set_color(c_white);
        draw_text_ext(text_x, text_y, item_data.description, line_height, tooltip_width - 5); // 20→5
    }
}
...
draw_set_color(c_ltgray);
draw_set_alpha(0.7);
draw_text(ui_x + ui_width / 2, ui_y + ui_height - 2, "Tap an item to purchase"); // 10→2
draw_set_alpha(1);
