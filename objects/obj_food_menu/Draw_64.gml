//if (global.inventory.item_has("Lemon", 1)){
//    instance_create_layer(x, y, "Instances", obj_food_basic);
//}
...
draw_rectangle(
    ...
    ui_padding_top + ui_padding_y + ui_border_size,
    (g_width - ui_padding_x) + ui_border_size,
    ui_padding_top + (g_height - ui_padding_y) + ui_border_size,
    false
)
...
draw_rectangle(
    ui_padding_x,
    ui_padding_top + ui_padding_y,
    (g_width - ui_padding_x),
    ui_padding_top + (g_height - ui_padding_y),
    false
);
...
for (var row = 0; row < inventory_rows; row++){
    var pos_y = ui_padding_top + ui_padding_y + (ui_border_size * 5) + (row * (ui_inventory_margin + ui_inventory_box));
